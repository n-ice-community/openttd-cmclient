 ...
 * @return the cost of this operation or an error
 */
CommandCost CmdBuildAirport(DoCommandFlags flags, TileIndex tile, uint8_t airport_type, uint8_t layout, StationID station_to_join, bool adjacent)
{
	bool reuse = (station_to_join != NEW_STATION);
	if (!reuse) station_to_join = StationID::Invalid();
	bool distant_join = (station_to_join != StationID::Invalid());
	...
	Station *st = nullptr;
	ret = FindJoiningStation(StationID::Invalid(), station_to_join, adjacent, airport_area, &st);
	if (ret.Failed()) return ret;
